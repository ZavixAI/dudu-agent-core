...
from core.http.context import clear_request_context, set_request_context
...
def _record_http_metrics(
    ...
    *,
    ...
    metrics.inc_counter("dudu_agent_core_http_requests_total", labels=labels)
    metrics.observe("dudu_agent_core_http_request_duration_ms", duration_ms, labels=labels)
    if failed:
        metrics.inc_counter("dudu_agent_core_http_requests_failed_total", labels=labels)
...
def register_middlewares(app) -> None:
    ...
    async def request_context_middleware(request: Request, call_next):
        ...
        if metrics_enabled:
            metrics.inc_gauge("dudu_agent_core_http_inflight_requests")
        ...
        try:
            ...
            if metrics_enabled:
                metrics.dec_gauge("dudu_agent_core_http_inflight_requests")
            clear_request_context()
