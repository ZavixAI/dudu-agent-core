...
class MetricsRegistry:
    ...
    def _collect_process_metrics(self) -> tuple[dict[MetricKey, float], dict[MetricKey, float]]:
        ...
        counters: dict[MetricKey, float] = {
            ("dudu_agent_core_process_cpu_seconds_total", ()): cpu_time,
        }
        gauges: dict[MetricKey, float] = {
            ("dudu_agent_core_process_cpu_percent", ()): cpu_percent,
            ("dudu_agent_core_process_start_time_seconds", ()): self._process_started_at,
        }
        rss_bytes, virtual_bytes = _read_process_memory_bytes()
        if rss_bytes is not None:
            gauges[("dudu_agent_core_process_resident_memory_bytes", ())] = float(rss_bytes)
        if virtual_bytes is not None:
            gauges[("dudu_agent_core_process_virtual_memory_bytes", ())] = float(virtual_bytes)
